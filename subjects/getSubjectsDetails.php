<?
	...
	if ($_SESSION['role']==2)
	{
		...
		$getSubjects_details_querylector = mysql_query("SELECT lector_name,lector_id FROM lectors where lector_id in (select lector_id from subjects where subject_id in (select subject_id from groups_subjects where subject_id=".$subjectId."))");		
