...
while ($rowStudent = mysql_fetch_array($resultStudent)) {
    ...
    for ($j = 0; $j < $res_col_mod; $j++) {
        ...
        $date = mysql_query(
            "SELECT * FROM groups_modules WHERE group_id=" . $groupId . " AND module_id=" . $rowModule['module_id']
        );
        $date_array = mysql_fetch_array($date);
        $grades[$j] = array(
            'module_id' => $rowModule['module_id'],
            'module_name' => $rowModule['module_name'],
            'grade' => $rowGrades['grade'],
            'date' => $date_array['dead_line']
        );
    }
    ...
    if ((($_SESSION['id']) == ($rowLectors['lector_id'])) && ($_SESSION['role'] == 2)) {
        $mas_student[$i] = array(
            'id_student' => $rowStudent['student_id'],
            'name_student' => $rowStudent['student_name'],
            'moduls' => $grades,
            'subject_credits' => $rowSubject['subject_credits']
        );
        $a = array(
            'extraProps' => array(
                'roleKey' => 2,
